<?php
    session_start();
    include"../pengaturan/koneksi.php"; //memanggil koneksi dg cara, naik 1 folder, lalu masukfolder pengaturan
    ...
    if($aksi=="tambah")
    {
        ...
        move_uploaded_file($sumber_gambar,"../gambar/".$gambar); // dipindahkan dari sumber kefolder gambar di server dengan nama yg sama seperti nama aslinya
        if($simpan)
    ...
    else if($aksi=="hapus")
    {
        ...
        $berita=mysqli_fetch_array(mysqli_query($koneksi,"select gambar from berita where id_berita='$id_berita'"));
        unlink('../gambar/'.$berita['gambar']); //menghapus file gambar di folder gambar
        $hapus=mysql_query("delete from berita where id_berita='$id_berita'");
        if($hapus)
